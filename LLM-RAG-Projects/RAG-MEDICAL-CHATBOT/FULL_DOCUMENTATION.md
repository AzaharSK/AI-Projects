...
## FastAPI + Observability + uv (Updated)
...
uv pip install -e .
uv run uvicorn app.application:app --host 0.0.0.0 --port 8000
```
...
- `/app-metrics` business metrics
- `/metrics` Prometheus scrape endpoint

### OpenTelemetry Export Configuration

Use an OTEL Collector to route telemetry:

- metrics -> Prometheus (scrape/export)
- logs -> OpenSearch
- traces/logs/metrics dashboards -> Grafana or Datadog
...
export OTEL_SERVICE_NAME="rag-medical-chatbot"
